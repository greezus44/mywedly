...
/**
 * RustyRsvp — RSVP per sub-event with cream/gold styling.
 */
    ...
    return (
      <div className="min-h-screen px-6 py-16" style={{ backgroundColor: "var(--event-bg)", color: "var(--event-text)" }}>
        <div className="max-w-lg mx-auto">
          ...
          <p className="text-center text-sm mb-2" style={{ ...scriptFont, color: "var(--event-text-muted)" }}>
            {event.name}
          </p>
          <GoldDivider />
          <RsvpForm
            ...
            venue={event.venue}
            invalidate={() => queryClient.invalidateQueries({ queryKey: ["rusty-rsvps", event.id] })}
          />
        </div>
...
function RsvpForm({
  ...
  const { data: existing } = useQuery({
    queryKey: ["rusty-rsvp", eventId, subEventId, guestName],
    queryFn: async () => {
  ...
  return (
    ...
    >
      ...
      {eventDate && (
        <p className="text-sm mb-4" style={{ ...scriptFont, color: "var(--event-text-muted)" }}>
