...
import { supabase, Wedding, LogoConfig } from "../../lib/supabase";
...
export default function BrandingEditor() {
  ...
  useEffect(() => {
    if (wedding) setLogoConfig(wedding.draft_logo_config || wedding.logo_config || DEFAULT_LOGO_CONFIG);
  }, [wedding?.id]);
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { error } = await supabase.from("weddings").update({ draft_logo_config: logoConfig }).eq("id", wedding.id);
      if (error) throw error;
      queryClient.setQueryData(["wedding"], (old: Wedding | null) => old ? { ...old, draft_logo_config: logoConfig } : old);
  ...
  }, [wedding, logoConfig, queryClient]);
    ...
    <div className="max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Logo and brand identity for your wedding website</p>
        </div>
      ...
      <Card className="p-6 space-y-5">
        <div>
          ...
          <div className="space-y-4">
            ...
            {logoConfig.enabled && (
              <>
                <FormField label="Logo Image" hint="Upload an image logo, or use text below">
                  <ImageUpload value={logoConfig.image} onChange={(v) => update({ image: v })} />
                </FormField>

                {!logoConfig.image && (
                  <FormField label="Logo Text">
                    <Input value={logoConfig.text} onChange={(e) => update({ text: e.target.value })} placeholder="e.g. W&M" />
                  </FormField>
                )}

                <FormField label="Logo Size">
                  <RangeInput value={logoConfig.fontSize} min={12} max={72} onChange={(v) => update({ fontSize: v })} />
                </FormField>
