...
import { supabase, Wedding, LoginConfig, LogoConfig } from "../../lib/supabase";
...
export default function LoginEditor() {
  ...
  const [config, setConfig] = useState<LoginConfig>(wedding?.draft_login_config || wedding?.login_config || DEFAULT_LOGIN_CONFIG);
  ...
  useEffect(() => {
    if (wedding) {
      setConfig(wedding.draft_login_config || wedding.login_config || DEFAULT_LOGIN_CONFIG);
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { error } = await supabase.from("weddings").update({ draft_login_config: config, draft_logo_config: logoConfig }).eq("id", wedding.id);
      if (error) throw error;
      queryClient.setQueryData(["wedding"], (old: Wedding | null) => old ? { ...old, draft_login_config: config, draft_logo_config: logoConfig } : old);
      setToast("Sign In page saved!");
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Sign In Page</h1>
          <p className="text-sm text-gray-500">Guests enter their name here to access your invitation</p>
        </div>
