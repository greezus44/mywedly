...
// Re-export the outlet context hook so rusty routes share the same context shape
          ...
          <nav className="absolute left-0 right-0 top-full z-20 border-b shadow-lg" style={{ backgroundColor: "var(--event-surface)", borderColor: "var(--event-border)" }}>
            <div className="mx-auto max-w-5xl px-4 py-3">
              <div className="flex flex-col gap-1">
                <MenuLink to={`/r/${slug}/home`} label="Home" onNavigate={() => setMenuOpen(false)} />
                {(invitedSubEventIds ?? []).length > 0 && (
                  <MenuLink to={`/r/${slug}/rsvp`} label="RSVP" onNavigate={() => setMenuOpen(false)} />
                )}
                <MenuLink to={`/r/${slug}/wishes`} label="Wishes" onNavigate={() => setMenuOpen(false)} />
                <MenuLink to={`/r/${slug}/contact`} label="Contact" onNavigate={() => setMenuOpen(false)} />
                {pages.map((p) => (
                  <MenuLink key={p.slug} to={`/r/${slug}/p/${p.slug}`} label={p.nav_label || p.title} onNavigate={() => setMenuOpen(false)} />
                ))}
              </div>
            </div>
          </nav>
