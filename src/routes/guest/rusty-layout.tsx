...
export default function RustyLayout() {
  ...
  const { data: customPages } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from("custom_pages").select("*").eq("event_id", event!.id).eq("is_published", true).order("sort_order", { ascending: true }); if (error) throw error; return data as CustomPage[]; },
    enabled: !!event?.id,
  });
        ...
        <header className="sticky top-0 z-40" style={{ borderBottom: "2px solid var(--event-border)", background: "var(--event-surface)" }}>
          <div className="max-w-5xl mx-auto px-4">
            <div className="h-16 flex items-center justify-between">
              ...
              {guestName && eventId === event.id ? (<div className="flex items-center gap-3"><span className="text-sm" style={{ color: "var(--event-muted)" }}>Hi, {guestName}</span><button onClick={handleSignOut} className="text-sm" style={{ color: "var(--event-muted)" }}><LogOut className="w-4 h-4" /></button></div>) : null}
            </div>
            <nav className="flex flex-wrap gap-1 pb-2">
              ...
              {navPages.map((page) => (<Link key={page.id} to={`/e/${slug}/p/${page.slug}`} className="px-3 py-1.5 rounded-full text-sm" style={{ color: "var(--event-text)" }}>{page.nav_label}</Link>))}
            </nav>
          </div>
        </header>
        <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-8"><Outlet context={{ event }} /></main>
        <footer className="border-t mt-8" style={{ borderColor: "var(--event-border)", background: "var(--event-surface)" }}>
          <div className="max-w-5xl mx-auto px-4 py-6">
            {footerPages.length > 0 && (<div className="flex flex-wrap gap-4 mb-4">{footerPages.map((page) => (<Link key={page.id} to={`/e/${slug}/p/${page.slug}`} className="text-sm" style={{ color: "var(--event-muted)" }}>{page.nav_label}</Link>))}</div>)}
