...
export default function RustyRsvp() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-semibold text-[var(--event-text)] mb-4" style={{ fontFamily: "var(--event-heading-font)" }}>
        RSVP
      </h1>

      {/* Event Details */}
      <div className="bg-[var(--event-surface)] border border-[var(--event-border)] rounded-lg p-4 mb-5">
        <h2 className="text-lg font-medium text-[var(--event-text)] mb-2" style={{ fontFamily: "var(--event-heading-font)" }}>
          {event.name}
        </h2>
        <div className="space-y-1 text-sm text-[var(--event-text-muted)]">
          {event.event_date && (
      ...
      {guest && (
        <form
          ...
          className="bg-[var(--event-surface)] border border-[var(--event-border)] rounded-lg p-4 space-y-4"
        >
      ...
      {guest && (
        <div className="mt-5 text-center">
          <button onClick={() => { signOut(); navigate(`/r/${slug}/signin`); }} className="text-xs text-[var(--event-text-muted)] hover:underline">
            Sign out
          </button>
        </div>
      )}
