...
export default function Rsvp() {
  ...
  const { data: groupAssignments } = useQuery({
    queryKey: ["guest_group_assignments_all", event.id],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const { data: overrides } = useQuery({
    queryKey: ["guest_invitation_overrides_all", event.id],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const invitedSubEvents = useMemo(() => {
    ...
    const invitedOverrideIds = new Set((overrides || []).filter((o) => o.is_invited).map((o) => o.sub_event_id));
    const removedOverrideIds = new Set((overrides || []).filter((o) => !o.is_invited).map((o) => o.sub_event_id));
    // Guest's group id - we don't have it directly, so include all events where any group is assigned
    // plus explicit overrides. For simplicity, show events with group assignments + manual adds.
    return subEvents.filter((se) => {
      if (removedOverrideIds.has(se.id)) return false;
      if (invitedOverrideIds.has(se.id)) return true;
  ...
  function getForm(subEventId: string): RsvpFormState {
    ...
    return { status: "", plus_ones: 0, dietary: "", message: "" };
  }

  function updateForm(subEventId: string, patch: Partial<RsvpFormState>) {
    ...
    return (
      <div className="max-w-md mx-auto text-center py-12">
        ...
        <p className="text-event-muted">
          The RSVP deadline ({event.rsvp_deadline ? formatDateTime(event.rsvp_deadline) : ""}) has passed.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      {invitedSubEvents.map((se) => {
        ...
        return (
          <div key={se.id} className="event-card space-y-4">
            <h3 className="font-event text-xl text-event-heading">{se.name}</h3>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => updateForm(se.id, { status: "attending" })}
                className={`event-btn-primary flex-1 ${form.status === "attending" ? "" : "opacity-50"}`}
              >
                Accept
              </button>
              <button
                type="button"
                onClick={() => updateForm(se.id, { status: "not_attending" })}
                className={`event-btn-secondary flex-1 ${form.status === "not_attending" ? "" : "opacity-50"}`}
              >
                Decline
              </button>
            </div>
            ...
            <button
              type="button"
              disabled={!form.status || submitMutation.isPending}
              onClick={() => submitMutation.mutate(se.id)}
