...
export default function GuestHome() {
  ...
  // Public page reads from event.content (published field)
        ...
        return (
          <section key={i} className="guest-section">
            <div className="mx-auto max-w-3xl">
              {headingText && (
                <h2 className="guest-title mb-4" style={headingStyle}>{headingText}</h2>
              )}
              {/* FIX #2: render raw HTML to preserve inline font/color/alignment styles */}
              {section.body && (
                <div className="rich-content" dangerouslySetInnerHTML={{ __html: section.body }} />
              )}
            </div>
          </section>
        );
