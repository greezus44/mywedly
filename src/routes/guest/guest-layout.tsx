...
import { jsonToTheme, type ThemeConfig as ThemeConfigType } from "../../lib/theme";
...
export default function GuestLayout() {
  ...
  const menuItems = [
    ...
    { to: `/e/${slug}/wishes`, label: "Wishes" },
    { to: `/e/${slug}/contact`, label: "Contact" },
    ...headerPages.map((p) => ({ to: `/e/${slug}/p/${p.slug}`, label: p.nav_label || p.title })),
  ];
    ...
    <EventThemeProvider theme={event.theme}>
      <div className="min-h-screen">
        <div ref={menuRef} className="fixed left-4 top-4 z-50">
          <button onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation menu" aria-expanded={menuOpen} aria-controls="guest-nav-menu"
          ...
          {menuOpen && (
            <nav id="guest-nav-menu" role="menu" aria-label="Guest navigation" className="absolute left-0 top-14 w-64 origin-top-left animate-scaleIn rounded-2xl py-2 shadow-xl" style={{ backgroundColor: "var(--event-surface)", border: "1px solid var(--event-border)" }}>
              {menuItems.map((item) => (
