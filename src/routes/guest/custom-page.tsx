...
function BlockRenderer({ block }: { block: ContentBlock }) {
  ...
  switch (block.type) {
    case "heading":
      return <h2 className="text-2xl font-serif" style={{ color: "var(--event-primary)" }}>{d.text || ""}</h2>;
    case "subheading":
      return <h3 className="text-lg event-muted-text">{d.text || ""}</h3>;
    case "richtext":
      return <RichTextContent html={d.html || ""} className="text-base leading-relaxed" />;
    case "image":
      return d.url ? <img src={d.url} alt="" className="w-full rounded-xl" /> : null;
    case "gallery":
      return (
    ...
    case "button":
      return d.url ? <div className="text-center"><a href={d.url} target="_blank" rel="noopener" className="inline-block px-6 py-2.5 rounded-lg text-white font-medium" style={{ background: "var(--event-primary)" }}>{d.label || "Click Here"}</a></div> : null;
    case "quote":
      return (
    ...
    case "faq":
      return <FaqAccordion items={d.items || []} />;
    case "timeline":
      return (
