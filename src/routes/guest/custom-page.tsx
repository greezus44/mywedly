    ...
    <div>
      {(page.title || page.cover_image_url) && (
        <section className="guest-section text-center">
          {page.cover_image_url && <img src={page.cover_image_url} alt={page.title} className="mx-auto mb-6 max-h-96 w-full rounded-2xl object-cover" />}
          {page.title && <h1 className="guest-title">{page.title}</h1>}
        </section>
      )}
      {blocks.length > 0 ? <BlockRenderer blocks={blocks} /> : page.body ? (
        <section className="guest-section"><div className="mx-auto max-w-2xl rich-content" dangerouslySetInnerHTML={{ __html: page.body }} /></section>
      ) : null}
    </div>
  );
}
