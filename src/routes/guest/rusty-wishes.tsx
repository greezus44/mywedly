...
export default function RustyWishes() {
  ...
  const { data: messages } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from("event_messages").select("*").eq("event_id", event.id).order("created_at", { ascending: false }); if (error) throw error; return data as EventMessage[]; },
  });
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="space-y-3">
        {messages?.map((m) => (<div key={m.id} className="p-4 rounded-lg" style={{ background: "var(--event-surface)", border: "1px solid var(--event-border)" }}><p className="text-sm mb-1">{m.message}</p><p className="text-xs event-muted-text">— {m.guest_name} on {formatDateShort(m.created_at)}</p></div>))}
      </div>
    </div>
  );
}
