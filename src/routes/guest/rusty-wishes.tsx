...
export default function RustyWishes() {
  ...
  const { data: messages, isLoading } = useQuery({
    queryKey: ["rusty_event_messages", event.id],
    queryFn: async () => {
  ...
  const postMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["rusty_event_messages", event.id] });
      setMessage("");
      setError(null);
    },
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit} className="event-card border-2 border-event-border space-y-3">
        <div>
          <label className="block text-sm font-medium text-event-text mb-1">Your Message</label>
          <textarea rows={4} value={message} onChange={(e) => setMessage(e.target.value)} className="event-input" placeholder="Write your wish..." />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {messages.map((msg) => (
            <div key={msg.id} className="event-card border-2 border-event-border">
