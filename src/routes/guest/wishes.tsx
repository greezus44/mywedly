...
/**
 * GuestWishes — guest wishes/messages. Shows a list of submitted messages
 * and a form to submit a new one. Uses event CSS vars for all colors.
 */
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10" style={{ color: "var(--event-text-muted)" }}>
            <p className="text-sm">No wishes yet. Be the first to share your love!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {messages.map((msg) => (
              <div
                ...
                style={{ borderColor: "var(--event-border)", backgroundColor: "var(--event-surface)", borderRadius: "var(--event-radius)" }}
              >
                <p className="text-sm leading-relaxed mb-3" style={{ color: "var(--event-text)" }}>
                  {msg.message}
                </p>
                <p className="text-xs uppercase tracking-wider" style={{ color: "var(--event-text-muted)" }}>
                  — {msg.guest_name}
                </p>
              </div>
            ))}
