...
export default function GuestCover() {
  ...
  // FIX #3: the cover page has NO form/keyboard handler — clicking Enter button
  // navigates to sign-in. There is no form element, so pressing Enter on the page
  // cannot accidentally bypass authentication.
  const goToSignIn = () => navigate(`/e/${slug}/signin`);

  return (
    <EventThemeProvider theme={event.theme}>
      {/* No <form> wrapper — prevents Enter key from submitting anything */}
      <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden" style={bgStyle}>
        ...
        <div className="relative z-10 flex w-full max-w-lg flex-col items-center px-6 py-16 text-center animate-fadeIn">
          ...
          {/* type="button" prevents implicit form submission on Enter */}
          <button type="button" onClick={goToSignIn} className="event-btn-primary">
            {buttonText}
          </button>
        </div>
      </div>
