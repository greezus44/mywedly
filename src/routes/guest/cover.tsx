...
export default function Cover() {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-event-bg">
        <div className="animate-pulse text-event-muted">Loading…</div>
      </div>
    );
  }
  ...
  return (
    <EventThemeProvider initialTheme={theme}>
      <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        {event.cover_image ? (
          <>
            <img src={event.cover_image} alt="Cover" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="relative z-10 flex w-full max-w-md flex-col items-center px-6 py-12 text-center">
          ...
          <form onSubmit={handleSubmit} className="mt-8 w-full space-y-3 rounded-xl bg-event-surface/95 p-6 shadow-xl backdrop-blur">
            ...
            <button type="submit" disabled={submitting} className="event-btn-primary w-full">
