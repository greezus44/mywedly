...
/**
 * GuestContact — contact and venue information page.
 * Shows event venue, address, date/time, and a map placeholder.
  ...
  return (
    <div className="min-h-screen px-6 py-16" style={{ backgroundColor: "var(--event-bg)", color: "var(--event-text)" }}>
      <div className="max-w-2xl mx-auto">
        ...
        >
          <h2 className="text-2xl mb-6" style={headingFont}>{event.name}</h2>
            ...
            {event.event_date && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm" style={{ color: "var(--event-text)" }}>{formatDate(event.event_date)}</p>
                </div>
              </div>
            )}
            {event.event_time && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm" style={{ color: "var(--event-text)" }}>{formatTime(event.event_time)}</p>
                </div>
              </div>
            )}
            {event.venue && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm" style={{ color: "var(--event-text)" }}>{event.venue}</p>
                </div>
              </div>
            )}
            {event.address && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm" style={{ color: "var(--event-text)" }}>{event.address}</p>
                </div>
              </div>
            )}
          </div>
          ...
          {(event.venue || event.address) && (
            <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-6">
              <Button
                variant="secondary"
                style={{ borderColor: "var(--event-border)", color: "var(--event-text)", borderRadius: "var(--event-radius)" }}
              >
        ...
        >
          ...
          >
            <div className="text-center">
              ...
              <p className="text-sm" style={scriptFont}>Map preview</p>
