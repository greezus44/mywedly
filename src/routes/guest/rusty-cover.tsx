...
export default function RustyCover() {
  ...
  // Rusty theme CSS vars — override even when event.theme differs
  const rustyVars = themeToEventCssVars(RUSTY_THEME) as React.CSSProperties;

  return (
    <EventThemeProvider theme={event.theme}>
      <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden" style={{ ...bgStyle, ...rustyVars }}>
        {bgConfig.image && (
        ...
        <div className="relative z-10 flex w-full max-w-lg flex-col items-center px-6 py-16 text-center animate-fadeIn">
          {logoConfig.url && (
            <div className="mb-8 w-full flex" style={{ justifyContent: logoAlign === "left" ? "flex-start" : logoAlign === "right" ? "flex-end" : "center" }}>
              {/* Transparent logo — mix-blend-mode for rustic overlay */}
              <img
                src={logoConfig.url}
                alt="Logo"
                style={{ height: `${logoSize}px`, width: "auto", maxHeight: "40vh", mixBlendMode: "screen", opacity: 0.9 }}
          ...
          {/* NO date/location — rustic cover keeps it minimal */}
