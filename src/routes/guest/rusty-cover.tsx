...
function parseCoverConfig(cfg: Json | null | undefined): CoverConfig {
  if (cfg && typeof cfg === "object" && !Array.isArray(cfg)) return cfg as CoverConfig;
  return {};
}

function parseLoginConfig(cfg: Json | null | undefined): LoginConfig {
  if (cfg && typeof cfg === "object" && !Array.isArray(cfg)) return cfg as LoginConfig;
  ...
  if (isError || !event) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-dash-bg px-4 text-center">
        ...
        <Link to="/" className="text-dash-primary underline">Go to homepage</Link>
      </div>
    );
  }
      ...
      <div className="flex min-h-screen flex-col">
        {event.cover_image ? (
          <div className="relative flex-1">
            ...
            <div className="relative flex min-h-screen flex-col items-center justify-end px-4 pb-16 text-center text-white">
              <h1 className="text-4xl font-bold drop-shadow sm:text-5xl" style={{ fontFamily: "var(--event-font-heading)" }}>
                {event.name}
              </h1>
              {coverConfig.subtitle && <p className="mt-3 text-lg text-white/90 drop-shadow">{coverConfig.subtitle}</p>}
              {event.event_date && <p className="mt-2 text-base text-white/90 drop-shadow">{formatDate(event.event_date)}</p>}
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-4 py-16 text-center">
            ...
            {coverConfig.subtitle && <p className="mt-3 text-lg opacity-80">{coverConfig.subtitle}</p>}
        ...
        <div className="mx-auto w-full max-w-sm px-4 py-10">
          <div className="event-card space-y-4">
            ...
            <p className="text-center text-sm opacity-80">Please enter your username to find your invitation.</p>
