...
export default function RustyCover() {
  ...
  return (
    <EventThemeProvider theme={event.theme}>
      <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden" style={{ ...rustyVars, ...bgStyle }}>
        ...
        <div className="relative z-10 flex w-full max-w-lg flex-col items-center px-6 py-16 text-center animate-fadeIn">
          {logoConfig.url && (
            <div className="mb-8 flex w-full" style={{ justifyContent: logoAlign === "left" ? "flex-start" : logoAlign === "right" ? "flex-end" : "center" }}>
              <img src={logoConfig.url} alt="Logo" style={{ height: `${logoSize}px`, width: "auto", maxHeight: "40vh", filter: "brightness(0) invert(1)", opacity: 0.9 }} className="object-contain" />
            </div>
          )}
          {coverConfig.eyebrow && <p className="guest-eyebrow mb-2">{coverConfig.eyebrow}</p>}
          {(coverConfig.heading || event.name) && <h1 className="guest-title mb-3">{coverConfig.heading || event.name}</h1>}
          {coverConfig.subheading && <p className="guest-subtitle mb-3">{coverConfig.subheading}</p>}
          {coverConfig.bodyHtml && <div className="rich-content mb-8 max-w-md" dangerouslySetInnerHTML={{ __html: coverConfig.bodyHtml }} />}
