    ...
    <EventThemeProvider initialTheme={theme}>
      <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 flex w-full max-w-md flex-col items-center px-6 py-16 text-center animate-fadeIn">
          {logoConfig.url && <img src={logoConfig.url} alt="Logo" className="mb-8 h-16 w-auto object-contain opacity-90" />}
          <p className="guest-eyebrow" style={{ color: "var(--event-primary-fg)" }}>{event.event_type || "Invitation"}</p>
          <h1 className="mb-4 text-4xl font-bold leading-tight md:text-5xl" style={{ color: "var(--event-primary-fg)" }}>{event.name}</h1>
          {event.event_date && <p className="mb-2 text-lg opacity-90" style={{ color: "var(--event-primary-fg)" }}>{formatDate(event.event_date)}</p>}
          {coverConfig.subtitle && <p className="mb-10 text-base opacity-80" style={{ color: "var(--event-primary-fg)" }}>{coverConfig.subtitle}</p>}
          <form onSubmit={handleSubmit} className="w-full space-y-4 rounded-2xl border border-white/20 bg-white/10 p-6 backdrop-blur-md" style={{ borderColor: "var(--event-primary-fg)", opacity: 0.95 }}>
            <div className="text-left">
              <label className="mb-1.5 block text-sm font-medium" style={{ color: "var(--event-primary-fg)" }}>Username</label>
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter your username" className="event-input" style={{ backgroundColor: "rgba(255,255,255,0.95)" }} required />
            </div>
            {loginConfig.passwordMode && loginConfig.passwordMode !== "none" && (
              <div className="text-left">
                <label className="mb-1.5 block text-sm font-medium" style={{ color: "var(--event-primary-fg)" }}>Password</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" className="event-input" style={{ backgroundColor: "rgba(255,255,255,0.95)" }} required />
              </div>
            )}
            {error && <p className="text-sm text-red-300">{error}</p>}
            <button type="submit" disabled={submitting} className="event-btn-primary w-full disabled:opacity-50">{submitting ? "Signing in..." : (loginConfig.signInText || "Enter")}</button>
          </form>
        </div>
      </div>
    </EventThemeProvider>
