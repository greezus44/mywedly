...
export default function RustyHome() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4">
      {/* Logo */}
      {typeof logoConfig.imageUrl === "string" && (
      ...
      {/* RSVP section */}
      <div className="rsvp-section text-center py-4 mt-3 rounded-lg">
        <h2 className="text-xl font-semibold text-[var(--event-text)] mb-2" style={{ fontFamily: "var(--event-heading-font)" }}>
          {event.event_date && formatDate(event.event_date)}
        </h2>
        {event.event_time && (
          <p className="text-sm text-[var(--event-text-muted)] mb-2">{formatTime(event.event_time)}</p>
        )}
        {event.venue && (
          <p className="text-sm text-[var(--event-text-muted)] mb-2">{event.venue}</p>
        )}
        <button
          onClick={() => navigate(`/r/${slug}/rsvp`)}
