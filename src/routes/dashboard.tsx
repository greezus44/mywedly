...
import { supabase, UserEvent, EVENT_TYPES, EventType, EVENT_TEMPLATES } from "../lib/supabase";
...
export default function Dashboard() {
  ...
  const createMutation = useMutation({
    mutationFn: async (input: { name: string; event_type: EventType; event_date: string | null; template: string }) => {
      const { data, error } = await supabase.from("user_events").insert({
        name: input.name,
        event_type: input.event_type,
        event_date: input.event_date,
        draft_event_date: input.event_date,
        draft_cover_config: DEFAULT_COVER_CONFIG,
        draft_login_config: DEFAULT_LOGIN_CONFIG,
        draft_theme: DEFAULT_THEME,
        draft_logo_config: DEFAULT_LOGO_CONFIG,
        draft_content: DEFAULT_CONTENT,
        draft_sharing_config: DEFAULT_SHARING_CONFIG,
      }).select().single();
      if (error) throw error;
      return data;
    },
  ...
  const archiveMutation = useMutation({
    mutationFn: async (event: UserEvent) => {
      const { error } = await supabase.from("user_events").update({ is_archived: !event.is_archived }).eq("id", event.id);
      if (error) throw error;
    },
      ...
      <main className="max-w-[1100px] mx-auto px-6 py-8">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search..."
                className="pl-8 pr-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:outline-none focus:border-gray-900 w-40 sm:w-56"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as any)}
              className="px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:outline-none focus:border-gray-900 bg-white"
            >
        ...
        {!isLoading && !error && filtered.length === 0 && (
          <EmptyState
            ...
            action={<Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4" /> Create Event</Button>}
          />
        )}
            ...
            {filtered.map(event => (
              <div key={event.id} className="group bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-gray-300 transition-colors">
                <div
                  ...
                  onClick={() => navigate(`/event/${event.id}/cover`)}
                >
                  {event.cover_image ? (
                  ...
                  <div className="absolute top-2 right-2">
                    {event.is_published ? <Badge variant="success">Published</Badge> : <Badge>Draft</Badge>}
                  </div>
                </div>
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="relative">
                      <button
                        onClick={(e) => { e.stopPropagation(); setMenuOpen(menuOpen === event.id ? null : event.id); }}
                      ...
                      {menuOpen === event.id && (
                        <>
                          ...
                          <div className="absolute right-0 top-full mt-1 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-20 py-1">
                            <button onClick={() => { duplicateMutation.mutate(event); setMenuOpen(null); }} className="w-full px-3 py-1.5 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                              <Copy className="w-3.5 h-3.5" /> Duplicate
                            </button>
                            <button onClick={() => { archiveMutation.mutate(event); setMenuOpen(null); }} className="w-full px-3 py-1.5 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                              <Archive className="w-3.5 h-3.5" /> {event.is_archived ? "Unarchive" : "Archive"}
                            </button>
                            <button onClick={() => { deleteMutation.mutate(event.id); setMenuOpen(null); }} className="w-full px-3 py-1.5 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
...
function CreateEventModal({ onClose, onCreate, creating }: { onClose: () => void; onCreate: (input: { name: string; event_type: EventType; event_date: string | null; template: string }) => void; creating: boolean }) {
  const [name, setName] = useState("");
  const [eventType, setEventType] = useState<EventType>("wedding");
  const [date, setDate] = useState<string | null>(null);
  const [template, setTemplate] = useState("wedding-classic");

  return (
    <Modal open={true} onClose={onClose} title="Create New Event">
      <div className="space-y-4">
        <FormField label="Event Name">
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Sarah & John's Wedding" autoFocus />
        </FormField>
            ...
            {EVENT_TEMPLATES.map(t => (
              <button
                ...
                className={cn(
                  ...
                  template === t.id ? "border-black bg-gray-50" : "border-gray-200 hover:border-gray-300"
                )}
              >
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button onClick={() => onCreate({ name: name || "Untitled Event", event_type: eventType, event_date: date, template })} loading={creating} disabled={!name.trim()}>
            Create Event
          </Button>
        </div>
      </div>
