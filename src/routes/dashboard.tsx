...
export default function DashboardPage() {
  ...
  const [showCreate, setShowCreate] = useState(false); const [deleteId, setDeleteId] = useState<string | null>(null); const [toast, setToast] = useState<string | null>(null);
  ...
  const { data: events, isLoading } = useQuery({
    ...
    queryFn: async () => { if (!session?.user?.id) return []; const { data, error } = await supabase.from("user_events").select("*").eq("creator_id", session.user.id).order("created_at", { ascending: false }); if (error) throw error; return data as UserEvent[]; },
    enabled: !!session?.user?.id,
  });
  const createMutation = useMutation({
    mutationFn: async () => { const { data, error } = await supabase.from("user_events").insert({ name: newEvent.name, event_type: newEvent.event_type, template_id: newEvent.template_id, creator_id: session?.user?.id, theme: newEvent.template_id === "rusty" ? RUSTY_THEME : {} }).select().single(); if (error) throw error; return data; },
  ...
  if (!session) return (<div className="min-h-screen bg-white"><SiteHeader /><div className="max-w-md mx-auto px-6 py-20 text-center"><h1 className="font-heading text-3xl mb-4">Sign in to continue</h1><p className="text-sm text-gray-500 mb-8">You need an account to manage your events.</p><Link to="/auth"><Button>Sign In</Button></Link></div></div>);
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        ) : !events || events.length === 0 ? (<EmptyState icon={<Calendar className="w-12 h-12" />} title="No events yet" description="Create your first wedding invitation to get started." action={<Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4" /> Create Event</Button>} />
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((event) => { const status = getEventStatus(event.draft_event_date || event.event_date); return (
              <Card key={event.id} className="overflow-hidden hover:shadow-md transition-shadow group">
                <div className="h-40 bg-gray-100 relative overflow-hidden">{event.cover_image ? <img src={event.cover_image} alt={event.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-gray-900"><span className="font-heading text-4xl text-white">{event.name.charAt(0)}</span></div>}<div className="absolute top-3 right-3"><Badge variant={event.is_published ? "success" : "default"}>{event.is_published ? "Published" : "Draft"}</Badge></div></div>
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2"><Link to={`/event/${event.id}`}><Button size="sm" variant="secondary"><Pencil className="w-3.5 h-3.5" /> Edit</Button></Link>{event.is_published && event.slug && <Link to={`/e/${event.slug}`} target="_blank"><Button size="sm" variant="ghost"><Eye className="w-3.5 h-3.5" /> View</Button></Link>}<button onClick={() => setDeleteId(event.id)} className="ml-auto p-2 text-gray-400 hover:text-red-600 transition-colors"><Trash2 className="w-4 h-4" /></button></div>
                </div>
              </Card>
            ); })}
          </div>
