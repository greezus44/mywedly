...
export default function HomeEditor() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (newContent: EventContent) => {
      const { error } = await supabase.from("user_events").update({ draft_content: newContent }).eq("id", event.id);
    ...
    <div>
      ...
      <SplitEditor preview={<HomePreview event={{ ...event, draft_content: local } as UserEvent} />}>
        <Card className="p-4 space-y-4">
          <FormField label="Title Section">
            <RichTextEditor value={local.title || ""} onChange={(html) => update({ title: html })} placeholder="Enter title..." minHeight={80} />
          </FormField>
          <FormField label="Subtitle Section">
            <RichTextEditor value={local.subtitle || ""} onChange={(html) => update({ subtitle: html })} placeholder="Enter subtitle..." minHeight={80} />
          </FormField>
          <FormField label="Body Section">
            <RichTextEditor value={local.body || ""} onChange={(html) => update({ body: html })} placeholder="Enter body content..." minHeight={200} />
          </FormField>
        </Card>
      </SplitEditor>
    </div>
