  ...
  const duplicateMutation = useMutation({
    mutationFn: async (page: CustomPage) => {
      const { data, error } = await supabase.from("custom_pages").insert({
        ...
        nav_label: page.nav_label + " (Copy)",
        slug: slugify(page.title + "-copy"),
        icon: page.icon,
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Create additional pages like About Us, Accommodation, FAQ, Travel Info, and more."
          action={<Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4" /> New Page</Button>}
        />
      ) : (
            ...
            <tbody>
              {filtered.map((page) => (
                <tr key={page.id} className="border-b border-dash-border hover:bg-slate-50">
                  ...
                  <td className="py-3 px-3">
                    <Badge variant={page.is_published ? "success" : "default"}>{page.is_published ? "Published" : "Draft"}</Badge>
                  </td>
                  <td className="py-3 px-3">
                    {page.show_in_nav ? (
                      <span className="inline-flex items-center gap-1 text-sm text-dash-muted"><Eye className="w-3 h-3" /> Visible</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-sm text-dash-muted"><EyeOff className="w-3 h-3" /> Hidden</span>
                    )}
                  </td>
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {event.is_published && page.is_published && (
                        <a href={`/e/${event.slug}/p/${page.slug}`} target="_blank" rel="noopener" className="p-1.5 text-dash-muted hover:text-dash-primary rounded hover:bg-slate-100" title="Preview"><Globe className="w-4 h-4" /></a>
                      )}
