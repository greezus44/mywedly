...
export default function CoverEditor() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (newCfg: CoverConfig) => {
      const { error } = await supabase.from("user_events").update({ draft_cover_config: newCfg }).eq("id", event.id);
      ...
      <SplitEditor preview={<CoverPreview event={{ ...event, draft_cover_config: local } as UserEvent} />}>
        <Card className="p-4 space-y-4">
          <FormField label="Title">
            <Input value={local.title || ""} onChange={(e) => update({ title: e.target.value })} placeholder="Event title" />
          </FormField>
          <FormField label="Subtitle">
            <Input value={local.subtitle || ""} onChange={(e) => update({ subtitle: e.target.value })} placeholder="Subtitle" />
          </FormField>
          <FormField label="Cover Image">
            <ImageUpload value={local.cover_image || null} onChange={(url) => update({ cover_image: url })} eventId={event.id} />
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Date">
              <DatePicker value={local.date || null} onChange={(d) => update({ date: d })} />
            </FormField>
            <FormField label="Time">
              <TimePicker value={local.time || null} onChange={(t) => update({ time: t })} />
            </FormField>
          </div>
          <FormField label="Venue">
            <Input value={local.venue || ""} onChange={(e) => update({ venue: e.target.value })} placeholder="Venue name" />
          </FormField>
        </Card>
      </SplitEditor>
    </div>
