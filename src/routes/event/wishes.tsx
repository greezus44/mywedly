...
export function WishesPage() {
  ...
  const [wishesContent, setWishesContent] = useState<WishesContent>(() => {
    ...
    return (raw.wishes as WishesContent | undefined) ?? {};
  });
  const [saving, setSaving] = useState(false);
  const [saveSuccess, setSaveSuccess] = useState(false);

  // FIX #1: Load messages — event_messages has guest_name, not guest_id
  ...
  const saveSettings = async () => {
    ...
    const existing = (event.draft_content as Record<string, unknown> | null) ?? (event.content as Record<string, unknown> | null) ?? {};
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setTab("messages")} className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${tab === "messages" ? "bg-dash-primary/10 text-dash-primary" : "text-dash-muted hover:text-dash-text"}`}>Messages {messages ? `(${messages.length})` : ""}</button>
          <button onClick={() => setTab("settings")} className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${tab === "settings" ? "bg-dash-primary/10 text-dash-primary" : "text-dash-muted hover:text-dash-text"}`}>Page Settings</button>
        </div>
      </div>
      ...
      {tab === "messages" && (
        <>
          ...
          : isError ? <ErrorState title="Failed to load wishes" message={error instanceof Error ? error.message : "Unknown error"} />
          : !messages || messages.length === 0 ? (
            <div className="rounded-lg border border-dash-border bg-dash-surface p-8 text-center"><p className="text-dash-muted">No wishes submitted yet.</p></div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="mb-1 flex items-center gap-2">
                        <span className="text-sm font-medium text-dash-text">{msg.guest_name}</span>
                    ...
                    <button onClick={() => setDeleteTarget(msg.id)} className="shrink-0 rounded p-1 text-dash-muted transition-colors hover:bg-red-50 hover:text-dash-danger" title="Delete wish">
      ...
      {tab === "settings" && (
        <div className="space-y-4 rounded-lg border border-dash-border bg-dash-surface p-4">
          ...
          <div>
            ...
            <input type="text" value={wishesContent.heading ?? ""} onChange={(e) => setWishesContent((p) => ({ ...p, heading: e.target.value }))} placeholder="Wishes & Messages" className="w-full rounded-lg border border-dash-border bg-dash-bg px-3 py-2 text-sm text-dash-text focus:border-dash-primary focus:outline-none" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={wishesContent.submitLabel ?? ""} onChange={(e) => setWishesContent((p) => ({ ...p, submitLabel: e.target.value }))} placeholder="Send Wish" className="w-full rounded-lg border border-dash-border bg-dash-bg px-3 py-2 text-sm text-dash-text focus:border-dash-primary focus:outline-none" />
          </div>
