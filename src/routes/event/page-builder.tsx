...
const BLOCK_TYPES: { type: BlockType; label: string; icon: any }[] = [
  { type: "heading", label: "Heading", icon: Type }, { type: "subheading", label: "Subheading", icon: Type },
  { type: "richtext", label: "Rich Text", icon: Type }, { type: "image", label: "Image", icon: Image },
  { type: "gallery", label: "Image Gallery", icon: Image }, { type: "divider", label: "Divider", icon: Minus },
  { type: "button", label: "Button", icon: Link }, { type: "quote", label: "Quote", icon: Quote },
  { type: "countdown", label: "Countdown", icon: Clock }, { type: "maps", label: "Google Maps", icon: MapPin },
  { type: "video", label: "Video Embed", icon: Video }, { type: "contact", label: "Contact Info", icon: Mail },
  { type: "social", label: "Social Links", icon: Share2 }, { type: "spacer", label: "Spacer", icon: Minus },
  { type: "two-col", label: "Two Columns", icon: Columns }, { type: "three-col", label: "Three Columns", icon: Columns },
  { type: "faq", label: "FAQ Accordion", icon: HelpCircle }, { type: "timeline", label: "Timeline", icon: Calendar },
...
export default function PageBuilder() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (blocks: ContentBlock[]) => { const { error } = await supabase.from("custom_pages").update({ blocks, updated_at: new Date().toISOString() }).eq("id", pageId); if (error) throw error; },
  ...
  const updatePageMetaMutation = useMutation({
    mutationFn: async (updates: Partial<CustomPage>) => { const { error } = await supabase.from("custom_pages").update({ ...updates, updated_at: new Date().toISOString() }).eq("id", pageId); if (error) throw error; },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["custom-page", pageId] }),
    onError: (err: any) => alert("Failed to save: " + (err.message || "Unknown error")),
  });
  ...
  React.useEffect(() => {
    if (page) { setBlocks(page.blocks || []); setTitle(page.title); setNavLabel(page.nav_label); setSlug(page.slug); setIcon(page.icon || ""); setShowInNav(page.show_in_nav); setIsFooter(page.is_footer); }
  }, [page]);
  ...
  const updateBlock = (id: string, data: Record<string, any>) => { setBlocks(blocks.map((b) => b.id === id ? { ...b, data: { ...b.data, ...data } } : b)); };
  const removeBlock = (id: string) => { setBlocks(blocks.filter((b) => b.id !== id)); };
  const moveBlock = (index: number, dir: "up" | "down") => { const newBlocks = [...blocks]; const target = dir === "up" ? index - 1 : index + 1; if (target < 0 || target >= newBlocks.length) return; [newBlocks[index], newBlocks[target]] = [newBlocks[target], newBlocks[index]]; setBlocks(newBlocks); };
  const handleDragStart = (index: number) => setDragIndex(index);
  const handleDragOver = (e: React.DragEvent, index: number) => { e.preventDefault(); };
  const handleDrop = (index: number) => { if (dragIndex === null || dragIndex === index) return; const newBlocks = [...blocks]; const [moved] = newBlocks.splice(dragIndex, 1); newBlocks.splice(index, 0, moved); setBlocks(newBlocks); setDragIndex(null); };
    ...
    <div>
      ...
      <div className="space-y-3 mb-4">
        {blocks.map((block, index) => (
          <div key={block.id} draggable onDragStart={() => handleDragStart(index)} onDragOver={(e) => handleDragOver(e, index)} onDrop={() => handleDrop(index)} className="border border-dash-border rounded-lg bg-white p-4">
      ...
      {blocks.length === 0 && (<div className="text-center py-12 text-dash-muted border-2 border-dashed border-dash-border rounded-lg"><p className="mb-2">No content blocks yet</p><p className="text-sm">Add your first block to start building this page</p></div>)}
      ...
      <Modal open={showAddBlock} onClose={() => setShowAddBlock(false)} title="Add Content Block" size="lg">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {BLOCK_TYPES.map((bt) => (
            <button key={bt.type} onClick={() => addBlock(bt.type)} className="flex flex-col items-center gap-2 p-4 border border-dash-border rounded-lg hover:border-dash-primary hover:bg-dash-primary-light transition-colors">
...
function BlockEditor({ block, onChange, eventId }: { block: ContentBlock; onChange: (data: Record<string, any>) => void; eventId: string }) {
  const d = block.data;
  switch (block.type) {
    case "heading": return <Input value={d.text || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ text: e.target.value })} placeholder="Heading text" />;
    case "subheading": return <Input value={d.text || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ text: e.target.value })} placeholder="Subheading text" />;
    case "richtext": return <RichTextEditor value={d.html || ""} onChange={(html) => onChange({ html })} placeholder="Write content..." minHeight={120} />;
    case "image": return <ImageUpload value={d.url || null} onChange={(url) => onChange({ url })} eventId={eventId} />;
    case "gallery": return (
    ...
    case "divider": return <p className="text-sm text-dash-muted">A horizontal divider line will appear here.</p>;
    case "button": return (
    ...
    case "countdown": return <Input type="date" value={d.date || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ date: e.target.value })} placeholder="Target date" />;
    case "maps": return (
      <div className="space-y-2">
    ...
    case "video": return <Input value={d.url || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ url: e.target.value })} placeholder="YouTube or Vimeo URL" />;
    case "contact": return (
      <div className="space-y-2">
    ...
    default: return <p className="text-sm text-dash-muted">Unknown block type</p>;
  }
}
