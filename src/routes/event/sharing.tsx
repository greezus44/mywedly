...
export default function SharingEditor() {
  ...
  React.useEffect(() => {
    generateQrDataUrl(guestUrl, 200).then(setQrUrl);
  }, [slug]);

  const saveMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("user_events").update({ draft_slug: slug }).eq("id", event.id);
  ...
  const copyLink = () => { navigator.clipboard.writeText(guestUrl); };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-4 space-y-4">
          <FormField label="Custom URL Slug" hint="Letters, numbers, and hyphens only. At least 3 characters.">
            <Input value={slug} onChange={(e) => setSlug(slugify(e.target.value))} placeholder="my-event" />
          </FormField>
          <div className="flex items-center gap-2">
            ...
            <Button variant="secondary" size="sm" onClick={copyLink}><Copy className="w-4 h-4" /></Button>
          </div>
          ...
          {event.is_published && event.slug && (
            <a href={`/e/${event.slug}`} target="_blank" rel="noopener" className="block text-sm text-teal-700 hover:underline">
              <ExternalLink className="w-4 h-4 inline mr-1" /> Open Guest Page
            </a>
          )}
        </Card>
