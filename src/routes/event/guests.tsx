...
export default function GuestsEditor() {
  ...
  const { data: guests, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("event_guests").select("*").eq("event_id", event.id).order("created_at", { ascending: false });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("event_guests").insert({ event_id: event.id, ...form });
      if (error) throw error;
    },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { setForm({ name: "", email: "", phone: "", group_name: "", side: "" }); setEditingId(null); setShowModal(true); }}><UserPlus className="w-4 h-4" /> Add Guest</Button>
      </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Add Guest">
        <div className="space-y-4">
          ...
          <FormField label="Group"><Input value={form.group_name} onChange={(e) => setForm({ ...form, group_name: e.target.value })} /></FormField>
