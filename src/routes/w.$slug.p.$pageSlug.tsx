...
export function GuestCustomPage() {
  ...
  const slug = parts[2];
  const pageSlug = parts[4];
  const { wedding, loading, error } = useWedding(slug);
  const [page, setPage] = useState<CustomPage | null>(null);

  useEffect(() => {
    if (!wedding) return;
    supabase.from("custom_pages").select("*").eq("wedding_id", wedding.id).eq("slug", pageSlug).maybeSingle().then(({ data }) => {
      setPage(data as CustomPage | null);
    });
  }, [wedding, pageSlug]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-6 py-16 bg-parchment">
      <div className="max-w-2xl mx-auto text-center text-sepia">
        ...
        {page.body && (
          <div className="text-sm md:text-base tracking-wide leading-[2] whitespace-pre-line text-left">{page.body}</div>
        )}
        <div className="mt-12">
