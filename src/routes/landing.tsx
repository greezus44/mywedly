    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-32 pb-24 text-center">
        <div className="animate-fade-in-up">
          ...
          <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/auth" className="inline-flex items-center gap-2 px-8 py-3.5 bg-black text-white text-sm font-medium hover:bg-gray-800 transition-all rounded-md animate-fade-in">Start Building <ArrowRight className="w-4 h-4" /></Link>
            <Link to="/#features" className="inline-flex items-center gap-2 px-8 py-3.5 border border-gray-200 text-sm font-medium hover:border-gray-400 transition-all rounded-md">Explore Features</Link>
          </div>
        </div>
      </section>
      <section id="features" className="max-w-6xl mx-auto px-6 py-24 border-t border-gray-100">
        ...
        <div className="grid md:grid-cols-3 gap-px bg-gray-100 border border-gray-100">
          {[{ icon: Users, title: "Guest Management", desc: "Organize guests into groups, send bulk invitations, and track RSVPs per event. Manual overrides always take precedence." }, { icon: Calendar, title: "Multiple Events", desc: "Create unlimited events within one invitation — ceremony, reception, rehearsal dinner — each with its own RSVP and schedule." }, { icon: Palette, title: "Theme Customization", desc: "Full control over colors, fonts, and layout with live preview. Changes apply instantly to the guest invitation, never the dashboard." }].map((f) => (
            <div key={f.title} className="bg-white p-10 hover:bg-gray-50 transition-colors"><div className="w-10 h-10 mb-6 flex items-center justify-center border border-gray-200 rounded-md"><f.icon className="w-5 h-5 text-gray-600" /></div><h3 className="font-heading text-xl mb-3">{f.title}</h3><p className="text-sm text-gray-500 leading-relaxed">{f.desc}</p></div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 border-t border-gray-100">
        <div className="flex flex-wrap items-center justify-center gap-8 md:gap-16 text-gray-400">{["No code required", "Custom domain ready", "Real-time RSVPs", "Mobile optimized", "GDPR compliant"].map((t) => <div key={t} className="flex items-center gap-2 text-sm"><Check className="w-4 h-4 text-gray-300" />{t}</div>)}</div>
      </section>
      <section id="pricing" className="max-w-6xl mx-auto px-6 py-24 border-t border-gray-100">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
          {[{ name: "Free", price: "$0", desc: "For small celebrations", features: ["1 event", "Up to 50 guests", "Basic themes", "Custom URL"], highlight: false }, { name: "Pro", price: "$29", desc: "For most weddings", features: ["Unlimited events", "Unlimited guests", "All themes", "QR codes & sharing", "Analytics"], highlight: true }, { name: "Studio", price: "$99", desc: "For planners & venues", features: ["Everything in Pro", "Multiple invitations", "Team access", "Priority support", "White-label option"], highlight: false }].map((p) => (
            <div key={p.name} className={`p-8 border rounded-lg ${p.highlight ? "border-black bg-black text-white" : "border-gray-200 bg-white"}`}>
              ...
              <Link to="/auth" className={`block text-center px-6 py-2.5 text-sm font-medium transition-all rounded-md ${p.highlight ? "bg-white text-black hover:bg-gray-100" : "border border-gray-200 hover:border-gray-400"}`}>Get Started</Link>
            </div>
          ))}
        </div>
      </section>
      <section className="bg-black text-white"><div className="max-w-6xl mx-auto px-6 py-24 text-center"><h2 className="font-heading text-3xl md:text-5xl tracking-tight mb-6">Begin your story.</h2><p className="text-gray-400 max-w-lg mx-auto mb-10">Create your wedding website in minutes. No design skills required.</p><Link to="/auth" className="inline-flex items-center gap-2 px-8 py-3.5 bg-white text-black text-sm font-medium hover:bg-gray-100 transition-all rounded-md">Create Your Event <ArrowRight className="w-4 h-4" /></Link></div></section>
      <SiteFooter />
    </div>
  );
}
