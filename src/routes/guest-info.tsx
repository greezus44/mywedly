  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center text-center px-6 py-16 bg-parchment">
      <div className="max-w-2xl">
        <h1 className="text-2xl md:text-3xl tracking-[0.35em] font-medium mb-12 uppercase text-sepia" style={styleFor(getStyle(content, "info_heading"))}>{heading.toUpperCase()}</h1>
        {image && <img src={image} alt="" className="max-w-full mx-auto mb-10 rounded-md" />}
        {body ? <div className="text-sepia text-sm tracking-wide leading-[2.4] font-medium whitespace-pre-line mb-16" style={styleFor(getStyle(content, "info_body"))}>{body}</div> : <p className="text-sepia/60 italic mb-16" style={{ fontFamily: "var(--font-serif)" }}>Additional information can be added from the dashboard.</p>}
