...
type Props = { slug: string; page?: "cover" | "invitation" | "info" | "events" };
...
export function WebsitePreview({ slug, page = "cover" }: Props) {
  ...
  if (mobile) {
    return (
      <>
        <button onClick={() => setMobileOpen(true)} className="fixed bottom-6 right-6 z-50 bg-onyx text-parchment px-4 py-3 text-xs uppercase tracking-widest shadow-lg flex items-center gap-2 hover:bg-ink"><Eye className="w-4 h-4" /> Preview</button>
        {mobileOpen && (
          <div className="fixed inset-0 z-50 bg-onyx/80 flex flex-col">
            <div className="flex justify-between items-center p-4 bg-parchment border-b border-onyx/10"><span className="text-xs uppercase tracking-widest text-onyx">Live Preview</span><button onClick={() => setMobileOpen(false)} className="text-onyx/60 hover:text-onyx"><X className="w-5 h-5" /></button></div>
