...
export function QrCodePanel({ url, title }: Props) {
  ...
  useEffect(() => {
    if (canvasRef.current) QRCode.toCanvas(canvasRef.current, url, { width: 200, margin: 2 }, (err) => { if (err) console.error(err); });
  }, [url]);
  const download = () => { if (!canvasRef.current) return; const link = document.createElement("a"); link.download = `${title.replace(/\s+/g, "-").toLowerCase()}-qr.png`; link.href = canvasRef.current.toDataURL(); link.click(); };
  const copyLink = async () => { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  const share = async () => { if (navigator.share) { try { await navigator.share({ title, url }); } catch {} } };
  return (
    <div className="border border-onyx/10 bg-card p-6 rounded-md">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="flex gap-2 flex-wrap justify-center">
          ...
          {typeof navigator !== "undefined" && "share" in navigator && <button onClick={share} className="flex items-center gap-1 text-sepia text-sm hover:text-onyx"><Share2 className="w-4 h-4" /> Share</button>}
        </div>
      </div>
    </div>
  );
