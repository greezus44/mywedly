      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-3">
          {(["desktop", "tablet", "mobile"] as Device[]).map((d) => {
            ...
            const Icon = icons[d];
            return <button key={d} onClick={() => setDevice(d)} className={cn("p-2 rounded-lg transition-colors", device === d ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200")}><Icon className="w-4 h-4" /></button>;
          })}
        </div>
        <div className="flex justify-center bg-slate-50 rounded-xl border border-slate-200 p-4 overflow-hidden">
          <div style={{ width: deviceWidths[device], maxWidth: "100%" }} className="transition-all duration-300">
            <div className="bg-white rounded-lg shadow-md overflow-hidden border border-slate-200">{preview}</div>
          </div>
        </div>
      </div>
    </div>
