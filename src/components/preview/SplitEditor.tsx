...
import { Monitor, Tablet, Smartphone, Maximize2, Minimize2, RotateCw, ZoomIn, ZoomOut, Edit3, Eye } from "lucide-react";
...
function DeviceFrame({ device, orientation, children }: { device: DeviceType; orientation: Orientation; children: ReactNode }) {
  ...
  if (device === "desktop") {
    return (
      <div className="device-browser-bar" style={{ width: dims.w }}>
        ...
        <div className="flex-1 ml-3 h-6 bg-white rounded-md border border-gray-200 flex items-center px-3 text-xs text-gray-400 font-ui">
          https://wedding-invitation.com
        </div>
        <div style={{ width: dims.w - 200, height: dims.h }} className="overflow-y-auto bg-[var(--color-bg)]">
          {children}
        </div>
      </div>
    );
  ...
  if (device === "tablet") {
    return (
      <div className="device-tablet-frame" style={{ width: dims.w + 28, height: dims.h + 28 }}>
        <div style={{ width: dims.w, height: dims.h }} className="overflow-y-auto bg-[var(--color-bg)] rounded-sm">
          {children}
        </div>
      </div>
    );
      ...
      {devices.map(({ type, icon: Icon, label }) => (
        <button
          ...
          className={cn(
            ...
            device === type ? "bg-white shadow-sm text-[var(--color-primary)]" : "text-gray-500 hover:text-gray-700"
          )}
        >
          <Icon size={14} />
...
export function OrientationToggle({ orientation, onChange, disabled }: { orientation: Orientation; onChange: (o: Orientation) => void; disabled?: boolean }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs font-ui font-medium text-gray-600 transition-all"
    >
      <RotateCw size={14} />
    ...
    <div className="inline-flex items-center gap-1 bg-gray-100 rounded-lg p-1">
      {levels.map((l) => (
        <button
          ...
          onClick={() => onChange(l)}
          className={cn(
            "px-2.5 py-1.5 rounded-md text-xs font-ui font-medium transition-all",
            zoom === l ? "bg-white shadow-sm text-[var(--color-primary)]" : "text-gray-500 hover:text-gray-700"
          )}
        >
          {l === "fit" ? "Fit" : `${l}%`}
        </button>
...
export function FullscreenPreview({ children, onClose }: { children: ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-8 animate-fade-in">
      <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-white/10 text-white rounded-full hover:bg-white/20 transition-colors">
        <Minimize2 size={20} />
      </button>
      <div className="max-w-full max-h-full overflow-auto">{children}</div>
    </div>
  );
}
...
export function SplitEditor({
  ...
  const previewContent = (
    <div className="flex items-center justify-center p-4 bg-gray-50 min-h-full">
      <div
        style={zoomScale ? { transform: `scale(${zoomScale})`, transformOrigin: "center top" } : undefined}
      >
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between gap-2 px-4 py-2.5 bg-white border-b border-gray-100 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <DeviceSwitcher device={device} onChange={(d) => { setDevice(d); setOrientation("portrait"); }} />
          <OrientationToggle orientation={orientation} onChange={setOrientation} disabled={device === "desktop"} />
      ...
      <div className="lg:hidden flex items-center gap-1 p-1 bg-gray-100 border-b border-gray-100">
        <button
          ...
          className={cn("flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-xs font-ui font-medium transition-all", mobileView === "edit" ? "bg-white shadow-sm text-[var(--color-primary)]" : "text-gray-500")}
        >
          <Edit3 size={14} /> Edit
        </button>
        <button
          onClick={() => setMobileView("preview")}
          className={cn("flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-xs font-ui font-medium transition-all", mobileView === "preview" ? "bg-white shadow-sm text-[var(--color-primary)]" : "text-gray-500")}
        >
          <Eye size={14} /> Preview
        </button>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className={cn("w-full lg:w-[420px] lg:flex-shrink-0 overflow-y-auto p-6 bg-white border-r border-gray-100", mobileView === "edit" ? "block" : "hidden lg:block")}>
          {children}
        </div>
        <div className={cn("flex-1 overflow-y-auto bg-gray-50", mobileView === "preview" ? "block" : "hidden lg:block")}>
          {previewContent}
        </div>
      </div>
