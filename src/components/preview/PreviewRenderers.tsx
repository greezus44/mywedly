...
function applyTheme(theme: ThemeConfig): React.CSSProperties {
...
export function CoverPreview({ event, coverConfig }: { event: UserEvent; coverConfig: CoverConfig }) {
  ...
  return (
    <div
      className="relative min-h-[400px] flex flex-col items-center justify-center text-center px-6 py-12"
      style={{
    ...
    >
      ...
      <div className="relative z-10 space-y-4">
        ...
        {config.showDate && (event.draft_event_date || event.event_date) && (
          <p className="text-sm opacity-80">{formatDate(event.draft_event_date || event.event_date)}</p>
        )}
        {config.showCountdown && !countdown.isPast && (
          <div className="flex gap-4 text-center">
            {(["days", "hours", "minutes"] as const).map(k => (
              <div key={k}>
                <div className="text-xl font-bold">{countdown[k]}</div>
                <div className="text-xs opacity-60 uppercase">{k}</div>
              </div>
            ))}
          </div>
        )}
        <button className="px-6 py-2.5 rounded-lg text-sm font-medium mt-4" style={{ background: config.buttonColor, color: config.bgColor }}>
          {config.buttonText}
        </button>
      </div>
    </div>
...
export function LoginPreview({ loginConfig, logo }: { loginConfig: LoginConfig; logo?: LogoConfig }) {
  ...
  return (
    <div
      ...
      style={{ background: config.bgImage ? `url(${config.bgImage}) center/cover` : config.bgColor, color: config.textColor }}
    >
      ...
      <div className="relative z-10 w-full max-w-sm space-y-4 p-8 rounded-xl" style={{ background: config.cardBgColor, borderColor: config.borderColor }}>
        {config.showLogo && logoConfig.enabled && (
          <div className="text-center">
            {logoConfig.image ? <img src={logoConfig.image} alt="Logo" className="h-12 mx-auto" /> : <div className="text-2xl font-bold" style={{ color: logoConfig.color, fontSize: logoConfig.fontSize }}>{logoConfig.text}</div>}
          </div>
        )}
        <div className="text-center">
          <h2 className="font-bold" style={{ fontFamily: config.headingFont, fontSize: config.headingFontSize, fontWeight: config.headingWeight }}>{config.title}</h2>
          <p className="text-sm mt-1 opacity-70">{config.subtitle}</p>
        </div>
        <p className="text-sm text-center opacity-80">{config.welcomeMessage}</p>
        <div className="space-y-3">
          <input type="text" placeholder={config.inputPlaceholder} className="w-full px-4 py-2.5 rounded-lg text-sm" style={{ background: config.inputBgColor, color: config.textColor, border: `1px solid ${config.borderColor}` }} disabled />
          <button className="w-full py-2.5 rounded-lg text-sm font-medium" style={{ background: config.buttonColor, color: "#fff" }}>{config.buttonText}</button>
        </div>
      </div>
    </div>
  );
...
export function DoaPreview({ theme }: { theme: ThemeConfig }) {
  const t = { ...DEFAULT_THEME, ...theme };
  return (
    <div style={applyTheme(t)} className="min-h-[300px] p-8">
      <div className="max-w-md mx-auto text-center space-y-4">
        <h2 className="text-2xl font-bold" style={{ color: t.headingColor, fontFamily: t.headingFont }}>Wishes</h2>
        <p className="text-sm" style={{ color: t.bodyColor }}>Share your wishes and messages</p>
        <div className="space-y-2 text-left">
          <div className="p-3 rounded-lg border" style={{ borderColor: t.accentColor, background: t.bgColor }}>
            <p className="text-xs font-semibold" style={{ color: t.headingColor }}>John Doe</p>
            <p className="text-xs mt-1" style={{ color: t.bodyColor }}>Congratulations! Wishing you all the best.</p>
          </div>
        </div>
      </div>
    </div>
...
export function SendMessagePreview({ theme }: { theme: ThemeConfig }) {
  const t = { ...DEFAULT_THEME, ...theme };
  return (
    <div style={applyTheme(t)} className="min-h-[200px] p-8">
      <div className="max-w-md mx-auto space-y-3">
        <textarea placeholder="Write your message..." className="w-full p-3 rounded-lg text-sm" style={{ background: t.bgColor, color: t.bodyColor, border: `1px solid ${t.accentColor}`, borderRadius: t.buttonRadius }} rows={3} disabled />
        <button className="w-full py-2.5 rounded-lg text-sm font-medium" style={{ background: t.buttonBgColor, color: t.buttonTextColor, borderRadius: t.buttonRadius }}>Send Message</button>
      </div>
    </div>
  );
}

export function ContactPreview({ event, theme }: { event: UserEvent; theme: ThemeConfig }) {
  const t = { ...DEFAULT_THEME, ...theme };
  return (
    <div style={applyTheme(t)} className="min-h-[300px] p-8">
      <div className="max-w-md mx-auto text-center space-y-3">
        <h2 className="text-2xl font-bold" style={{ color: t.headingColor, fontFamily: t.headingFont }}>Contact</h2>
        {(event.draft_venue || event.venue) && <p className="text-sm" style={{ color: t.bodyColor }}>{event.draft_venue || event.venue}</p>}
        {(event.draft_address || event.address) && <p className="text-sm" style={{ color: t.bodyColor }}>{event.draft_address || event.address}</p>}
        {(event.draft_event_date || event.event_date) && <p className="text-sm" style={{ color: t.accentColor }}>{formatDate(event.draft_event_date || event.event_date)}</p>}
      </div>
    </div>
  );
