import type { Wedding, CoverConfig, LoginConfig, LogoConfig, ThemeConfig, WeddingContent, WeddingEvent } from "../../lib/supabase";
import {
  getLogoStyle, getLogoPositionClasses, loginToCssVars, coverToCssVars, themeToCssVars,
  shouldShowLogo, DEFAULT_COVER_CONFIG, DEFAULT_LOGIN_CONFIG, DEFAULT_THEME,
} from "../../lib/theme";
...
export function LogoRenderer({ logo, device, page }: { logo: LogoConfig; device: DeviceType; page: string }) {
  if (!shouldShowLogo(logo, page)) return null;
  ...
  return (
    <div className={`flex w-full ${pos.container}`} style={{ padding: logo.padding }}>
      <div className={pos.item} style={{ transform: `translate(${logo.offsetX}, ${logo.offsetY})` }}>
        <img src={logo.url} alt="logo" style={style} />
      </div>
    </div>
  );
}
...
export function CoverPreview({ wedding, device }: { wedding: Wedding | null; device: DeviceType }) {
  ...
  return (
    <div
      ...
      style={{ ...coverToCssVars(cover), ...bgStyle, backgroundSize: "cover", backgroundPosition: "center", minHeight: "500px", filter: `brightness(${cover.brightness}) blur(${cover.blur})` } as React.CSSProperties}
    >
      ...
      {cover.overlay.enabled && (
        <div className="absolute inset-0" style={{ background: cover.overlay.color, opacity: cover.overlay.opacity }} />
      )}
      <div className={`relative z-10 flex flex-col p-8 ${cover.layout.vertical_position === "top" ? "justify-start" : cover.layout.vertical_position === "bottom" ? "justify-end" : "justify-center"} ${cover.layout.content_alignment === "left" ? "items-start text-left" : cover.layout.content_alignment === "right" ? "items-end text-right" : "items-center text-center"}`} style={{ maxWidth: cover.layout.max_width, margin: "0 auto", width: "100%" }}>
        ...
        <h1 className="mt-4 font-heading" style={{ color: cover.typography.heading_color, fontSize: cover.typography.heading_size, fontWeight: cover.typography.heading_weight, letterSpacing: cover.typography.letter_spacing }}>
          {cover.branding.couple_name_one || "Name One"}
        </h1>
        <p className="font-script text-3xl" style={{ color: cover.typography.body_color }}>&</p>
        <h1 className="font-heading" style={{ color: cover.typography.heading_color, fontSize: cover.typography.heading_size, fontWeight: cover.typography.heading_weight }}>
          {cover.branding.couple_name_two || "Name Two"}
        </h1>
        {cover.show_date && (cover.branding.date || wedding?.wedding_date) && (
          <p className="mt-3 font-body" style={{ color: cover.typography.body_color, fontSize: cover.typography.body_size }}>
          ...
          <div className="mt-6 flex gap-4">
            {(["days", "hours", "minutes", "seconds"] as const).map((k) => (
              <div key={k} className="text-center">
                <div className="text-2xl font-heading" style={{ color: cover.typography.heading_color }}>{countdown[k]}</div>
                <div className="text-xs uppercase" style={{ color: cover.typography.body_color }}>{k}</div>
              </div>
            ))}
          </div>
        )}
        <button className="mt-8 font-body" style={{ background: cover.button.bg_color, color: cover.button.text_color, borderRadius: cover.button.border_radius, padding: `${cover.button.padding_y} ${cover.button.padding_x}` }}>
...
export function LoginPreview({ wedding, device }: { wedding: Wedding | null; device: DeviceType }) {
  ...
  return (
    <div
      className="relative flex flex-col overflow-hidden"
      style={{ ...loginToCssVars(login), ...bgStyle, backgroundSize: "cover", backgroundPosition: "center", minHeight: "500px", filter: `brightness(${login.brightness}) blur(${login.blur})` } as React.CSSProperties}
    >
      {bg.type === "video" && bg.video_url && <video src={bg.video_url} autoPlay loop muted className="absolute inset-0 h-full w-full object-cover" />}
      {login.overlay.enabled && (
        <div className="absolute inset-0" style={{ background: login.overlay.color, opacity: login.overlay.opacity }} />
      )}
      <div className={`relative z-10 flex flex-col p-6 ${vPosClass} ${alignClass}`} style={{ maxWidth: login.layout.max_width, margin: login.layout.margin, width: "100%", gap: login.layout.spacing, padding: login.layout.padding }}>
        {/* Logo */}
        {login.branding.logo.visible && login.branding.logo.url && (
          <img
            src={login.branding.logo.url}
            alt="logo"
            style={getLogoStyle(login.branding.logo, device)}
          />
        )}

        {/* Title */}
        <h1 className="font-heading" style={{ color: login.theme.text, fontSize: login.typography.heading_size, fontWeight: login.typography.heading_weight, letterSpacing: login.typography.letter_spacing }}>
          {login.text.title}
        </h1>

        {/* Subtitle */}
        {login.text.subtitle && (
          <p className="font-body" style={{ color: login.theme.text, fontSize: login.typography.body_size, opacity: 0.85 }}>
            {login.text.subtitle}
          </p>
        )}

        {/* Welcome Message */}
        {login.text.welcome_message && (
          <p className="font-body" style={{ color: login.theme.text, fontSize: login.typography.body_size, opacity: 0.7 }}>
            {login.text.welcome_message}
          </p>
        )}
        ...
        {login.language.enabled && (
          <div
            ...
            style={{
              borderRadius: login.language_selector.button_radius,
              border: `1px solid ${login.language_selector.border_color}`,
              overflow: "hidden",
          ...
          >
            {login.language.order.map((l) => (
              <button
                ...
                onClick={() => setLang(l)}
                style={{
                  padding: `${login.language_selector.button_padding_y} ${login.language_selector.button_padding_x}`,
                  fontSize: login.language_selector.font_size,
                  fontWeight: login.language_selector.font_weight,
                  borderRadius: login.language_selector.button_radius,
                  background: lang === l ? login.language_selector.active_bg : login.language_selector.inactive_bg,
                  color: lang === l ? login.language_selector.active_text : login.language_selector.inactive_text,
                  transition: "all 0.25s ease",
                  border: "none",
                  cursor: "pointer",
                }}
              >
                {login.language.labels[l]}
              </button>
            ))}
          </div>
        )}
        ...
        {/* Username Input */}
        {login.form.username_field.show_label && (
          <label className="font-body text-sm" style={{ color: login.theme.text, opacity: 0.8 }}>
            {login.form.username_field.label_text}
          </label>
        )}
        <input
          type="text"
          ...
          placeholder={login.text.username_placeholder}
          style={{
            width: login.form.input.width,
            height: login.form.input.height,
            borderRadius: login.form.input.border_radius,
            border: `1px solid ${login.form.input.border_color}`,
            background: login.form.input.background,
            color: login.form.input.text_color,
            fontSize: login.form.input.font_size,
            padding: login.form.input.padding,
            boxShadow: login.form.input.shadow,
            outline: "none",
          }}
          onFocus={(e) => (e.target.style.borderColor = login.form.input.focus_border_color)}
          onBlur={(e) => (e.target.style.borderColor = login.form.input.border_color)}
        />
        ...
        <button
          type="button"
          style={{
            width: login.form.button.width,
            height: login.form.button.height,
            borderRadius: login.form.button.border_radius,
            background: login.form.button.bg_color,
            color: login.form.button.text_color,
            fontSize: login.form.button.font_size,
            fontWeight: login.form.button.font_weight,
            boxShadow: login.form.button.shadow,
            border: "none",
            cursor: "pointer",
            transition: "background 0.25s ease",
          }}
          onMouseEnter={(e) => (e.currentTarget.style.background = login.form.button.hover_bg_color)}
          onMouseLeave={(e) => (e.currentTarget.style.background = login.form.button.bg_color)}
        >
          {login.text.button_text}
        </button>

        {/* Helper Text */}
        {login.text.helper_text && (
          <p className="font-body text-sm" style={{ color: login.theme.text, opacity: 0.6 }}>
            {login.text.helper_text}
          </p>
        )}

        {/* Footer */}
        {login.text.footer_message && (
          <p className="font-body text-sm" style={{ color: login.theme.text, opacity: 0.5, marginTop: "1rem" }}>
            {login.text.footer_message}
          </p>
        )}
      </div>
...
export function HomePreview({ wedding, device }: { wedding: Wedding | null; device: DeviceType }) {
  ...
  return (
    <div className="p-8" style={{ ...themeToCssVars(theme), background: "var(--color-bg)", color: "var(--color-text)", fontFamily: "var(--font-body)" } as React.CSSProperties}>
      ...
      <h1 className="font-heading text-center" style={{ color: "var(--color-primary)", fontSize: "var(--font-heading-size)" }}>
        {content.home_title || `${wedding?.couple_name_one || "Name"} & ${wedding?.couple_name_two || "Name"}`}
      </h1>
      ...
      {content.quran_verse && (
        <blockquote className="mt-6 text-center font-body italic" style={{ color: "var(--color-accent)", maxWidth: "500px", margin: "1.5rem auto" }}>
          "{content.quran_verse}"
          {content.quran_reference && <footer className="mt-1 text-sm">— {content.quran_reference}</footer>}
        </blockquote>
      )}
      {!countdown.isPast && (
        <div className="mt-6 flex justify-center gap-4">
          {(["days", "hours", "minutes", "seconds"] as const).map((k) => (
            <div key={k} className="text-center">
              <div className="text-2xl font-heading" style={{ color: "var(--color-primary)" }}>{countdown[k]}</div>
              <div className="text-xs uppercase" style={{ color: "var(--color-text-muted)" }}>{k}</div>
            </div>
          ))}
        </div>
      )}
