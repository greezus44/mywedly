  ...
  return (
    <div style={{ ...vars, ...bg, minHeight: "100vh" }} className="flex flex-col items-center justify-center text-center p-8 relative">
      ...
      <div className="relative" style={{ color: config.textColor || theme.textColor }}>
        ...
        <button className="mt-8 px-8 py-3 text-sm uppercase tracking-wider" style={{ backgroundColor: config.buttonColor || theme.primaryColor, color: "#fff", borderRadius: "var(--radius)" }}>{config.buttonText || "Enter"}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ ...vars, ...bg, minHeight: "100vh" }} className="flex flex-col items-center justify-center p-8">
      <div className="relative w-full max-w-sm text-center" style={{ color: config.textColor || theme.textColor }}>
        ...
        <input type="text" placeholder={config.inputPlaceholder || "Your name"} className="w-full px-4 py-2.5 text-sm border bg-white/80 mb-3" style={{ borderColor: theme.borderColor, borderRadius: "var(--radius)" }} />
        <button className="w-full px-6 py-2.5 text-sm uppercase tracking-wider" style={{ backgroundColor: config.buttonColor || theme.primaryColor, color: "#fff", borderRadius: "var(--radius)" }}>{config.buttonText || "Continue"}</button>
      </div>
    </div>
  );
}
...
export function HomePreview({ event }: { event: UserEvent }) {
  ...
  return (
    <div style={vars} className="min-h-screen bg-[var(--color-bg)]">
      ...
      {content.story && <div className="max-w-2xl mx-auto px-6 py-12 text-center" style={{ color: "var(--color-text)" }}><p className="font-script italic text-2xl mb-4 opacity-80">Our Story</p><p className="text-sm leading-relaxed opacity-80">{content.story}</p></div>}
      {content.invitation_body && <div className="max-w-2xl mx-auto px-6 py-12 text-center" style={{ color: "var(--color-text)" }}><h2 className="font-heading text-3xl mb-4">{content.invitation_title || "You're Invited"}</h2><p className="text-sm leading-relaxed opacity-80">{content.invitation_body}</p></div>}
    </div>
  );
}
    ...
    <div style={vars} className="min-h-screen bg-[var(--color-bg)] flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-md text-center" style={{ color: "var(--color-text)" }}>
        ...
        <div className="flex gap-3 justify-center">
          <button className="px-8 py-3 text-sm uppercase tracking-wider" style={{ backgroundColor: "var(--color-primary)", color: "var(--color-bg)", borderRadius: "var(--radius)" }}>Joyfully Accepts</button>
          <button className="px-8 py-3 text-sm uppercase tracking-wider border" style={{ borderColor: "var(--color-border)", color: "var(--color-text)", borderRadius: "var(--radius)" }}>Regretfully Declines</button>
        </div>
      </div>
    </div>
  );
