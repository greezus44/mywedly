...
export function Card({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]/20 shadow-[var(--shadow-soft)]", className)}>
      {children}
    </div>
  );
}
...
export function Badge({ children, variant = "default" }: { children: ReactNode; variant?: "default" | "success" | "warning" | "error" }) {
  ...
  return (
    <span className={cn("inline-flex items-center px-3 py-1 rounded-full font-ui text-xs font-medium", variants[variant])}>
      {children}
    </span>
  );
}
...
export function SectionTitle({ children, subtitle, className }: { children: ReactNode; subtitle?: string; className?: string }) {
  return (
    <div className={cn("text-center mb-8", className)}>
      <h2 className="font-heading text-2xl md:text-3xl text-[var(--color-text)] mb-2">{children}</h2>
      {subtitle && <p className="font-ui text-sm text-[var(--color-text-muted)]">{subtitle}</p>}
    </div>
  );
}
...
export function Toast({ message, type = "success", onClose }: { message: string; type?: "success" | "error"; onClose: () => void }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-fade-in-up">
      <div className={cn(
        "px-5 py-3 rounded-lg shadow-lg font-ui text-sm",
        type === "success" ? "bg-[var(--color-success)] text-white" : "bg-[var(--color-error)] text-white"
      )}>
        {message}
      </div>
    </div>
