  ...
  return (
    <div className="border border-dash-border rounded-lg overflow-hidden">
      <div className="rte-toolbar flex flex-wrap gap-1 p-2 border-b border-dash-border bg-slate-50">
        {tools.map((t, i) => (
          <button key={i} type="button" title={t.label} onClick={() => exec(t.cmd, t.val)} className="p-1.5 rounded hover:bg-slate-200 text-dash-text">
            <t.icon className="w-4 h-4" />
          </button>
        ))}
        <button type="button" title="Link" onClick={() => { const url = prompt("Enter URL:"); if (url) exec("createLink", url); }} className="p-1.5 rounded hover:bg-slate-200 text-dash-text">
      ...
      <div
        ...
        className="rte-content px-4 py-3 focus:outline-none text-dash-text"
        style={{ minHeight }}
      />
    </div>
  );
