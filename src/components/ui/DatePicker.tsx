        ...
        <div className="absolute z-50 mt-1 bg-white rounded-lg border border-slate-200 shadow-lg p-3 w-72">
          ...
          <div className="grid grid-cols-7 gap-1 text-xs">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return <button key={day} type="button" onClick={() => selectDay(day)} className={cn("w-8 h-8 rounded text-center hover:bg-slate-100 transition-colors", isSelected && "bg-slate-900 text-white hover:bg-slate-800")}>{day}</button>;
            })}
          </div>
