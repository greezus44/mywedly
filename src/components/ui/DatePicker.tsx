...
export function DatePicker({ value, onChange, label, placeholder = "Select date" }: DatePickerProps) {
  ...
  const selectDay = (day: number) => {
    onChange(new Date(viewYear, viewMonth, day).toISOString().split("T")[0]);
    setOpen(false);
  };
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 bg-[var(--color-surface)] border border-[var(--color-border)] shadow-lg p-3 w-72" style={{ borderRadius: "var(--radius)" }}>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={cn("w-8 h-8 text-center hover:bg-[var(--color-bg-subtle)] transition-colors", isSelected && "bg-[var(--color-primary)] text-[var(--color-bg)] hover:opacity-80")}
                >
                  {day}
                </button>
              );
          ...
          {value && (
            <button type="button" onClick={() => { onChange(null); setOpen(false); }} className="mt-2 w-full text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] flex items-center justify-center gap-1">
      ...
      <div className="bg-[var(--color-surface)] max-w-lg w-full max-h-[90vh] overflow-y-auto animate-scale-in" style={{ borderRadius: "var(--radius)" }} onClick={(e) => e.stopPropagation()}>
        {title && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
            <h2 className="text-lg font-heading text-[var(--color-text)]">{title}</h2>
            <button onClick={onClose} className="p-1 hover:bg-[var(--color-bg-subtle)]"><X className="w-5 h-5 text-[var(--color-text-muted)]" /></button>
          </div>
        )}
