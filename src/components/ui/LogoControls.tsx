  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mt-1.5 flex gap-2">
          {([["desktop", Monitor], ["tablet", Tablet], ["mobile", Smartphone]] as const).map(([d, Icon]) => (
            <button key={d} type="button" onClick={() => onChange({ ...logo })} className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm transition ${device === d ? "border-gray-900 bg-gray-100 text-gray-900" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}><Icon className="h-4 w-4" /> {d}</button>
          ))}
        </div>
      </div>
      <div className="space-y-3">
        ...
        <FormField label="Object Fit"><Select value={logo.objectFit} onChange={(e) => update({ objectFit: e.target.value as LogoConfig["objectFit"] })}><option value="contain">Contain</option><option value="cover">Cover</option><option value="fill">Fill</option></Select></FormField>
      </div>
      <div className="space-y-3">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {POSITIONS.map((p) => (
            <button key={p.value} type="button" onClick={() => update({ position: p.value })} className={`rounded-lg border py-2.5 text-lg transition ${logo.position === p.value ? "border-gray-900 bg-gray-100 text-gray-900" : "border-gray-300 text-gray-500 hover:bg-gray-50"}`}>{p.label}</button>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-3">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between"><h4 className="text-sm font-semibold text-gray-900">Drop Shadow</h4><Toggle checked={logo.dropShadow.enabled} onChange={(v) => update({ dropShadow: { ...logo.dropShadow, enabled: v } })} /></div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between"><h4 className="text-sm font-semibold text-gray-900">Glow Effect</h4><Toggle checked={logo.glow.enabled} onChange={(v) => update({ glow: { ...logo.glow, enabled: v } })} /></div>
        {logo.glow.enabled && (
          <div className="grid grid-cols-2 gap-3">
      ...
      <div className="space-y-3">
        ...
        <FormField label="Show on"><Select value={logo.showOnPages} onChange={(e) => update({ showOnPages: e.target.value as LogoConfig["showOnPages"] })}><option value="all-pages">All pages</option><option value="cover-only">Cover page only</option><option value="custom">Custom pages</option></Select></FormField>
        {logo.showOnPages === "custom" && (
          <div className="space-y-2">
            {PAGES.map((p) => (
              <label key={p} className="flex items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" checked={logo.customPages.includes(p)} onChange={(e) => { const pages = e.target.checked ? [...logo.customPages, p] : logo.customPages.filter((x) => x !== p); update({ customPages: pages }); }} className="rounded" />
                {p}
              </label>
            ))}
          </div>
      ...
      <button type="button" onClick={() => onChange({ ...DEFAULT_LOGO_CONFIG })} className="w-full rounded-lg border border-gray-300 py-2 text-sm text-gray-600 hover:bg-gray-50 transition">Reset to Default</button>
    </div>
  );
}
