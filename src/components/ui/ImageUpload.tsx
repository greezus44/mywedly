...
export function ImageUpload({ value, onChange, eventId, label, aspectRatio = "16/9" }: ImageUploadProps) {
  ...
  return (
    <div>
      ...
      {value && !uploading ? (
        <div className="relative group overflow-hidden border border-gray-200 rounded-lg" style={{ aspectRatio }}>
          <img src={value} alt="Preview" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100"><button type="button" onClick={handleRemove} className="p-2 bg-white/90 hover:bg-white rounded"><X className="w-4 h-4" /></button></div>
        </div>
      ) : uploading ? (
        <div className="flex flex-col items-center justify-center border border-dashed border-gray-200 bg-gray-50 p-8 rounded-lg" style={{ aspectRatio }}><Loader2 className="w-6 h-6 text-gray-400 animate-spin mb-2" /><span className="text-sm text-gray-500">Uploading... {progress}%</span><div className="w-full max-w-xs mt-3 h-1 bg-gray-200 rounded"><div className="h-full bg-gray-900 transition-all rounded" style={{ width: `${progress}%` }} /></div></div>
      ) : (
