...
export function ImageUpload({ value, onChange, eventId, label, aspectRatio = "16/9" }: ImageUploadProps) {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (value) {
          ...
          if (oldPath) { try { await removeImage(oldPath); } catch { /* ignore */ } }
        }
        onChange(result.url);
      } catch (err) {
  ...
  const handleRemove = useCallback(async () => {
    if (value) {
      ...
      if (path) { try { await removeImage(path); } catch { /* ignore */ } }
    }
    onChange("");
  }, [value, onChange]);
      ...
      {value && !uploading ? (
        <div className="relative group overflow-hidden border border-[var(--color-border)]" style={{ aspectRatio, borderRadius: "var(--radius)" }}>
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
            <button type="button" onClick={handleRemove} className="p-2 bg-white/90 hover:bg-white" style={{ borderRadius: "var(--radius)" }}>
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center border border-dashed border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-8" style={{ aspectRatio, borderRadius: "var(--radius)" }}>
          <Loader2 className="w-6 h-6 text-[var(--color-text-muted)] animate-spin mb-2" />
          <span className="text-sm text-[var(--color-text-muted)]">Uploading... {progress}%</span>
          <div className="w-full max-w-xs mt-3 h-1 bg-[var(--color-border)]">
        ...
        <div
          ...
          className={cn(
            "flex flex-col items-center justify-center border border-dashed cursor-pointer transition-colors p-8",
            dragOver ? "border-[var(--color-primary)] bg-[var(--color-bg-subtle)]" : "border-[var(--color-border)] hover:border-[var(--color-primary)] hover:bg-[var(--color-bg-subtle)]"
          )}
          style={{ aspectRatio, borderRadius: "var(--radius)" }}
        >
          <Upload className="w-6 h-6 text-[var(--color-text-muted)] mb-2" />
          <span className="text-sm text-[var(--color-text-muted)]">Click or drag to upload</span>
          <span className="text-xs text-[var(--color-text-muted)] mt-1">JPG, PNG, WebP</span>
        </div>
      )}
