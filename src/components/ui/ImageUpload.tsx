    ...
    <div className={className}>
      ...
      {value ? (
        <div className="relative inline-block">
          ...
          <button type="button" onClick={() => onChange(null)} className="absolute -right-2 -top-2 rounded-full bg-red-500 p-1 text-white shadow"><X className="h-3 w-3" /></button>
        </div>
      ) : (
        <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading} className="flex h-32 w-full items-center justify-center rounded-lg border-2 border-dashed border-gray-300 text-gray-500 hover:border-gray-400 hover:text-gray-600 transition">
          {uploading ? <span className="text-sm">Uploading...</span> : (<div className="flex flex-col items-center gap-1"><Upload className="h-6 w-6" /><span className="text-sm">Upload image</span></div>)}
        </button>
      )}
  ...
  return (
    <div className={className}>
      ...
      {value ? (
        ...
        <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading} className="flex h-32 w-full items-center justify-center rounded-lg border-2 border-dashed border-gray-300 text-gray-500 hover:border-gray-400 hover:text-gray-600 transition">
          {uploading ? <span className="text-sm">Uploading...</span> : (<div className="flex flex-col items-center gap-1"><Film className="h-6 w-6" /><span className="text-sm">Upload video</span></div>)}
        </button>
      )}
