...
export async function downloadQrSvg(text: string, filename = "qr-code.svg", size = 256): Promise<void> {
  const svg = await QRCode.toString(text, { type: "svg", width: size, margin: 1, errorCorrectionLevel: "M" });
