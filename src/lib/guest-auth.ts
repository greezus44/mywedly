...
export async function guestSignin(weddingId: string, username: string) {
  ...
  if (!data || data.length === 0) return { error: "Guest not found", session: null };
  const row = data[0];
  const session: GuestSession = { guestId: row.guest_id, weddingId: row.wedding_id, fullName: row.out_full_name };
