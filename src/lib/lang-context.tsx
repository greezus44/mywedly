...
interface LangContextValue { lang: Language; setLang: (l: Language) => void; t: (typeof translations)["en"] }
const LangContext = createContext<LangContextValue | undefined>(undefined);

export function LanguageProvider({ children }: { children: ReactNode }) {
  const [lang, setLangState] = useState<Language>("en");
  useEffect(() => { const stored = localStorage.getItem("lang") as Language | null; if (stored && (stored === "en" || stored === "ms")) setLangState(stored); }, []);
